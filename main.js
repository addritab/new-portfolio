...
var _proj=[
  {title:'Your Daily Dough',desc:'Currently in production. Your Daily Dough is a fortune cookie generator built around how you feel. You input your mood, fold a virtual fortune cookie, and receive an AI-generated message tailored to your emotional state — a small, playful ritual designed to brighten your day.',images:['your daily dough design/1.png','your daily dough design/2.png','your daily dough design/3.png','your daily dough design/4.png','your daily dough design/5.png','your daily dough design/6.png']},
  {title:'Notifi',desc:'A focus-first productivity tool built with HTML, CSS, JavaScript, and TypeScript as a final project for Google Code Next. Notifi combines a Pomodoro timer, daily checklist, and lo-fi player in one minimal interface — designed to hold just enough for one day without the overwhelm of tools like Notion. The intent was to make focus feel approachable, not like another system to maintain.',images:['notifi design/1.jpg','notifi design/2.jpg','notifi design/3.jpg']},
  {title:'Intellect',desc:'Congressional App Challenge 2024 winner. Intellect is a guidance platform built for incoming high school students who want to build the strongest path to college from day one. It surfaces internships, scholarships, and academic resources, paired with a built-in checklist so students can stay organized without losing track of what matters.',images:['intellect design/1.png','intellect design/2.png','intellect design/3.png','intellect design/4.png','intellect design/5.png']},
...
function _warnName(rowId,inputId){var row=document.getElementById(rowId);if(!row)return;row.classList.add('name-warn');var inp=document.getElementById(inputId);if(!inp)return;inp.focus();inp.addEventListener('focus',function(){row.classList.remove('name-warn');},{once:true});}
window.submitNote=function(){var col=_rlc();if(_ism){var nm=document.getElementById('g-name-sketch').value.trim();if(!nm){_warnName('name-row-sketch','g-name-sketch');return;}if(!_sc)return;var du=_sc.toDataURL();_notes.unshift({name:nm,sketch:du,color:col});}else{var nm=document.getElementById('g-name').value.trim(),ms=document.getElementById('g-msg').value.trim();if(!nm){_warnName('name-row-note','g-name');return;}if(!ms)return;_notes.unshift({name:nm,msg:ms,color:col});}_save(_notes);_rn();window.closeGuest();};
